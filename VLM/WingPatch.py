...
class WingPatch:
    ...
    def _apply_sections(self, root: Section, tip: Section, corners_x: np.ndarray, corners_y: np.ndarray, corners_z: np.ndarray, wing_root_chord: float, wing_semi_span: float):
        ...
        for i_sec in range(corners_x.shape[1]):
            ...
            corners_x[:, i_sec], corners_z[:, i_sec] = airfoil_root.get_camber_line(corners_x[:, i_sec], fc * wing_root_chord, twist_deg)
            # corners_x[:, i_sec] *= fc
            corners_x[:, i_sec] += x_offset
            # corners_z[:, i_sec] += airfoil_root.get_camber_line(corners_x[:, i_sec], fc * wing_root_chord)
