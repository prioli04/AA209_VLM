...
class Decambering_Iscold:
    ...
    def __init__(self, wing_mesh: WingPanels, params: Parameters):
        ...
        self._delta = np.zeros(len(self._airfoil_ids))
        self._delta_orig_id, self._delta_orig_val = 0, 0.0
    ...
    def interpolate_visc_Cl(self, alfa_deg: np.ndarray, sec_id: int):
        ...
        # Cl = interp1d(self._alfa_visc[foil_id], self._Cl_visc[foil_id], fill_value="extrapolate")
        # return Cl(alfa_deg)
    ...
    def compute_residuals(self, alfa_sec: np.ndarray, Cl_sec: np.ndarray):
        ...
        for i in range(len(alfa_sec)):
            Cl_visc[i] = self.interpolate_visc_Cl(np.rad2deg(alfa_sec[i]), i)
    ...
    def apply_smoothing(self, PI: float):
        ...
        for i in range(len(self._delta)):
            ...
            # if i == 0 or i == len(self._delta) - 1:
            #     new_delta[i] = self._delta[i]
            
            # else:
                # new_delta[i] = (self._delta[i] + 0.5 * PI * (self._delta[i - 1] + self._delta[i + 1])) / (1.0 + PI)
            new_delta[i] = (self._delta[i] + 0.5 * PI * (self._delta[id1] + self._delta[id2])) / (1.0 + PI)
