...
class Solver:
    def __init__(self, mesh: VLM.Mesh, params: VLM.Parameters):
        self._mesh = mesh
        self._params = params
        self._nx, self._ny = self._mesh.get_n_panels()
    ...
    def solve(self):
        ...
        self._Gammas = Gammas.reshape(self._nx, -1)
        self._w_ind = w_ind.reshape(self._nx, -1)
        
        self._post.compute_coefficients(self._mesh, self._params, self._Gammas, self._w_ind)
    ...
    def _compute_aerodynamic_influence(self):
        ...
        quarter_chords = self._mesh.get_quarter_chords()
        wake_corners = self._mesh.get_wake_corners()
        collocation_points = self._mesh.get_collocation_points()
        normals = self._mesh.get_normals()
    ...
    @staticmethod
    def _VORING(quarter_chords: VLM.Mesh.GridVector3, P: np.ndarray, i: int, j: int, Gamma: float, wake_corners: VLM.Mesh.GridVector3 | None = None):
    ...
    @staticmethod
    def _VORTXL(P: np.ndarray, P1: np.ndarray, P2: np.ndarray, Gamma: float):
        eps = 1e-6
